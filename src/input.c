...
#include "../include/config.h"  // Add this include
...
bool process_input(EDITOR* editor) {
    ...
    if(_kbhit()) {
        ...
        if(editor->state == normal) {
            // Get action for this key in normal mode
            EditorAction action = get_action_for_key(editor->config, ch, normal);
            ...
            switch (action) {
                ...
                case ACTION_GO_TO_TOP:
                    set_cursor_position(editor, 0, 0);
                    break;
                    
                case ACTION_GO_TO_BOTTOM:
                ...
                case ACTION_ENTER_COMMAND_MODE:
                    // The existing command mode code here...
                    CONSOLE_SCREEN_BUFFER_INFO csbi;
                    GetConsoleScreenBufferInfo(editor->current_buffer, &csbi);
                ...
                default:
                    // If no binding is found, do nothing
                    break;
            }
        } else if (editor->state == insert) {
            // For insert mode, handle special keys first
            if(ch == 0 || ch == 224) {
                ch = _getch(); // Get the actual key code
                
                // Map arrow keys to actions
                EditorAction action = ACTION_NONE;
                
                switch(ch) {
                    case 72: action = ACTION_MOVE_UP; break;    // Up arrow
                    case 80: action = ACTION_MOVE_DOWN; break;  // Down arrow
                    case 75: action = ACTION_MOVE_LEFT; break;  // Left arrow
                    case 77: action = ACTION_MOVE_RIGHT; break; // Right arrow
                }
                ...
                // Handle regular keys
                EditorAction action = get_action_for_key(editor->config, ch, insert);
                
                switch (action) {
                    case ACTION_EXIT_INSERT_MODE:
                        editor->state = normal;
                        break;
...
void execute_command(EDITOR* editor, COMMANDS cmd) {
    switch(cmd) {
        ...
        case cmd_config:{
            open_create_file("twp.conf", open);
            exit(0);
            break;
        }
    }
